<template>
  <nav class="mb-2 flex justify-between align-top md:mb-4">
    <RouterLink :to="{ name: 'home' }" v-if="!hideLogo">
      <Logo responsive></Logo>
    </RouterLink>
    <div class="flex grow items-start justify-end">
      <!-- New Note: only shown on home / search where sidebar is hidden -->
      <RouterLink v-if="showNewButton && !showSidebar" :to="{ name: 'new' }">
...
// Sidebar is shown on note/search routes — New Note button is in the sidebar there
const showSidebar = computed(
  () => !["login", "home"].includes(route.name),
);
